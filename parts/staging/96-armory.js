...
const STAND_GLB={};
for(const k of ['angelic','crimson','forest','ice','lava','rock','shadow','storm','void','wind']) fetchBytes(ASSET('armor-stand-'+k+'.glb')).then(buf=>new THREE.GLTFLoader().parse(buf,'',gltf=>{ try{ const root=gltf.scene||gltf.scenes[0]; const fit=fitModel(root,2.3); toonify(root,fit.scale); STAND_GLB['stand-'+k]=fit.wrap; STANDS.forEach(st=>st.item=null); refresh(); }catch(e){ console.warn('armor stand '+k,e); } },e=>console.warn('armor stand '+k,e))).catch(e=>console.warn('armor stand '+k+' load',e));
...
function build(st,it){ const g=new THREE.Group(); const col=itemColor(it), tint=mat(col), shine=basic(col);
  if(it.slot==='armor'){ const pk=Meta.packs&&Meta.packs.of(it), sk=pk&&pk.models&&pk.models.armor, tpl=sk&&STAND_GLB[sk];
