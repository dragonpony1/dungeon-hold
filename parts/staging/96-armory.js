...
// a great set's own dressed mannequin (Meshy), used in place of the plain wooden one when a stashed armor piece belongs to a
// set whose pack carries `models.armor` (e.g. 'stand-void') — ten are shipped (angelic/crimson/forest/ice/lava/rock/shadow/
// storm/void/wind), one set uses its own so far; the rest wait on their own addSet() entries to ever be picked
const STAND_GLB={};
// a set's mannequin is fetched the first time a stashed piece of that set needs it (the plain wooden stand shows until it lands), not all ten at start
const STAND_ASKED={};
function ensureStand(sk){ const k=String(sk).replace(/^stand-/,''); if(STAND_GLB[sk]||STAND_ASKED[k]) return; STAND_ASKED[k]=true; fetchBytes(ASSET('armor-stand-'+k+'.glb'),'first').then(buf=>new THREE.GLTFLoader().parse(buf,'',gltf=>{ try{ const root=gltf.scene||gltf.scenes[0]; const fit=fitModel(root,2.3); toonify(root,fit.scale); STAND_GLB['stand-'+k]=fit.wrap; STANDS.forEach(st=>st.item=null); refresh(); }catch(e){ console.warn('armor stand '+k,e); } },e=>console.warn('armor stand '+k,e))).catch(e=>console.warn('armor stand '+k+' load',e)); }
...
function build(st,it){ const g=new THREE.Group(); const col=itemColor(it), tint=mat(col), shine=basic(col);
  if(it.slot==='armor'){ const pk=Meta.packs&&Meta.packs.of(it), sk=pk&&pk.models&&pk.models.armor, tpl=sk&&STAND_GLB[sk]; if(sk&&!tpl) ensureStand(sk);
    if(tpl){ g.add(tpl.clone()); }   // a great set's own dressed mannequin (Meshy) — already fully coloured, no rarity tint needed
    else { g.add(M(G.cyl(.06,.08,1.0,7),wood,0,.55,0)); const torso=M(G.box(.56,.72,.34),tint,0,1.4,0); g.add(torso); g.add(M(G.box(.62,.16,.38),tint,0,1.82,0)); g.add(M(G.box(.5,.2,.3),dark,0,.98,0)); g.add(M(G.sph(.17,9,7),cream,0,2.08,0)); g.add(M(G.box(.14,.5,.14),tint,-.4,1.45,0)); g.add(M(G.box(.14,.5,.14),tint,.4,1.45,0)); } }
    ...
    if(window.__weapons&&window.__weapons.model){ const hm=window.__weapons.mount&&window.__weapons.mount(); const name=hm&&hm.staff&&window.__staff?window.__staff.staffFor(it):hm&&hm.bow&&window.__bow?window.__bow.bowFor(it):window.__weapons.swordFor(it); window.__weapons.model(name,root=>{ if(st.shown!==g) return; const box=root.userData.box||new THREE.Box3().setFromObject(root); const L=Math.max(.2,box.max.y-box.min.y), s=1.25/L; root.scale.setScalar(s); root.position.set(0,.2-box.min.y*s,.02); toonify(root,s); root.traverse(o=>{ if(o.isMesh&&!o.userData.isOL&&o.material&&o.material.emissive&&Meta.packs&&Meta.packs.of(it)){ o.material=o.material.clone(); o.material.emissive.set(Meta.packs.of(it).emissive||0); o.material.emissiveIntensity=.7; } }); g.add(root); }); } }
