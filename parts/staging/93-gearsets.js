...
addSet({name:'of the Void',ic:'🌌',col:0x8a3dff,css:'#c070ff',emissive:0x5a2bd0,minR:2,chance:w=>w>=4?Math.min(.15,.05+.01*(w-4)):0,valueMul:3,
  ...
  models:{sword:'holy',staff:'staff-void'}, art:{sword:'item-void-sword.png',staff:'item-void-staff.png',armor:'item-void-armor.png',charm:'item-void-charm.png',amulet:'item-void-amulet.png'},   // 2-D card art for the bag, the shop and the sheet (assets/); the emoji stands in until a file is there
...
function itemArt(it){ if(!it) return null; if(it.art) return it.art; const d=packOf(it); if(!d||!d.art) return null; let k=it.slot; if(k==='weapon'){ const hm=window.__heroes&&window.__heroes.pick(); k=(hm==='witch')?'staff':'sword'; } const n=d.art[k]||d.art[it.slot]; if(!n) return null; return /^(data:|https?:|\.\/|\/)/.test(n)?n:ASSET(n); }
