...
function fireBolt(kind,from,dir,speed,opts){ opts=opts||{}; const K=STAFF_KINDS[kind]||STAFF_KINDS.hazel; const g=new THREE.Group(); g.position.copy(from); const core=M(G.sph(.2,8,6),basic(0xffffff)); core.userData.noOL=true; g.add(core); const shell=new THREE.Mesh(new THREE.OctahedronGeometry(.34,0),basic(K.gem,{transparent:true,opacity:.8})); shell.userData.noOL=true; g.add(shell);
  ...
  const d=dir.clone().normalize(); g.lookAt(from.clone().add(d)); g.rotateY(PI); scene.add(g); BOLTS.push({g,d,v:speed||22,t:0,life:opts.life||1.8,kind,shell,gl,x:from.x,y:from.y,z:from.z,dmg:opts.dmg||0,hit:new Set()}); return g; }
...
{ const prevHit=hitCone; hitCone=function(){ const wo=window.__weapons.mounted(); if(!(wo&&/^staff-/.test(wo.name))) return prevHit(); const from=staffHeadWorld(wo); const fx=Math.sin(hero.yaw), fz=Math.cos(hero.yaw); const range=hero.reach||9; let best=null, bd=1e9;
    for(const e of enemies){ if(e.dead) continue; const dx=e.x-hero.x, dz=e.z-hero.z, d=Math.hypot(dx,dz); if(d>range+e.r||d<.01||(dx*fx+dz*fz)/d<.8) continue; if(d<bd){ bd=d; best=e; } }
    const dir=best?new THREE.Vector3(best.x-from.x,(best.y+best.h*.5)-from.y,best.z-from.z):new THREE.Vector3(fx,-.02,fz);
    fireBolt(wo.userData.kind,from,dir,26,{dmg:heroDmg(),life:(range+1)/26}); SFX.harpoon(); }; }
