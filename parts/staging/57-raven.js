// ===== THE RAVEN: a terminal that flies in during the build phase, perched near the crystal (which always stands at
// world (0,0), so this works on every map with no per-map wiring). Pops in fast when the phase turns to 'build' and
// pops out just as fast the moment the horn sounds or the run ends. For now E near it just opens the same character
// sheet Tab does — the I-menu tavern and the E-upgrade prompt elsewhere are untouched. More of the raven's own
// interface (beyond the sheet) is still to come.
(function(){
const NEAR=3.2; let RX=0, RZ=-2.0;   // backed up to the wall behind the crystal — found per map below, out of the way of foot traffic
(function findPerch(){ let z=-.5; while(z>-40&&!wallAt(0,z-.5)) z-=.5; RZ=z-.7; })();
...
fetchBytes(ASSET('raven.glb')).then(buf=>new THREE.GLTFLoader().parse(buf,'',gltf=>{ try{
    const root=gltf.scene||gltf.scenes[0]; const fit=fitModel(root,1.4); toonify(root,fit.scale);
...
function ravenY(){ return hgt[GOAL]+2.6+Math.sin(S.t*1.6)*.12; }
...
function ravenUpdate(dt){ if(!wrap) return;
  ...
  wrap.position.set(RX,ravenY(),RZ); wrap.rotation.y=PI+Math.sin(S.t*1.1)*.1; wrap.rotation.z=Math.sin(S.t*1.7)*.04;
