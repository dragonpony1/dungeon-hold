...
function rebuildVisuals(){ if(V.weapon&&V.weapon.parent) V.weapon.parent.remove(V.weapon); if(V.charm) scene.remove(V.charm); if(V.amulet) scene.remove(V.amulet); V.weapon=V.charm=V.amulet=null;
  ...
  if(w&&w.rarity>=1&&host){ const s=glow(RCOL[w.rarity],1,.45+.12*w.rarity); let at=host, y=0; if(useGLB&&GLBH){ let mn=null; host.traverse(o=>{ if(!mn&&/^(weapon|staff)Mount_\d+/.test(o.name)) mn=o; }); if(mn){ at=mn; y=(+mn.name.split('_')[1])*.4; } }   // on the weapon mount when the rig has one, 40% up the blade
