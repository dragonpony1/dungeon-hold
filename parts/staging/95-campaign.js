...
function testLine(){ let el=$('testline'); if(!el){ el=document.createElement('p'); el.id='testline'; el.className='mapline'; el.style.fontSize='12px'; el.style.letterSpacing='1px'; el.style.color='#bfae90'; el.style.gap='8px'; const fold=$('howto'); if(fold) fold.appendChild(el); else $('heroline').insertAdjacentElement('afterend',el); }   /* the testing line lives in the folded section since build 133: testers know where it is, a new player never sees it */
  el.innerHTML='🧪 testing: <button data-t="maps">'+(cleared()>=MAPS.length?'all maps unlocked ✓':'unlock all maps')+'</button><button data-t="mana">auto-mana: '+(TEST.autoMana?'ON':'off')+'</button><button data-t="gold">+1000 gold</button><button data-t="guide" title="show the map-one training guide again from step one">🎓 reset guide</button><button data-t="reload" title="reload the page past any cached copy (saves kept)">↻ fresh reload</button><button data-t="wipe" title="forget gear, gold, skills, hero and map progress, then reload fresh">wipe saves</button>';
