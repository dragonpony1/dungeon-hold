...
const tpl={}, loading={}; const PROC_WEAPONS={};   // name -> builder for weapons made in code (staffs), served like a loaded template                 // name -> template root (bounds in userData.box) / pending callbacks
...
function mountSword(name,tier,key){ const hm=heroMount(); if(!hm) return; loadSword(name,root=>{ if(W.key!==key) return;   // a newer request won
    ...
    else { gripY=box.min.y+(root.userData.gripF!==undefined?root.userData.gripF:GRIP_F)*L; tipY=box.max.y; s=(len*lenMul(tier))/(box.max.y-gripY); obj.scale.setScalar(s); obj.position.set(0,-gripY*s,0); }   // the grip point sits on the mount (in the fist); the blade runs up the mount's +Y
    // ink outline + toon shading like everything else; the outline thickness accounts for the mount scale and the hero's fit
    node.updateWorldMatrix(true,false); const worldPerUnit=s*node.getWorldScale(new THREE.Vector3()).x; if(root.userData.proc) outlineScaled(obj,worldPerUnit); else toonify(obj,worldPerUnit); { const m=/\|set:(.+)$/.exec(key); const pk=m&&Meta.packs&&Meta.packs.get(m[1]); if(pk) setTint(obj,pk); }
...
function weaponsUpdate(dt){ const hm=heroMount(); if(!hm){ if(W.obj) unmount(); W.key=''; return; } if(W.obj&&W.obj.parent&&W.obj.parent!==hm.node) unmount();   // the hero model changed: drop the old weapon at once, the new one follows when its model is ready
  whipAnim(dt); const it=gear.weapon, name=FORCE_WEAPON||(hm.whip?whipFor(it):swordFor(it)), tier=swordTier(it); const pk=Meta.packs&&Meta.packs.of(it); const key=name+'|'+tier+'|'+GLBH.label+(pk&&pk.emissive?'|set:'+pk.name:''); if(key===W.key&&W.obj&&W.obj.parent===hm.node) return; W.key=key; mountSword(name,tier,key); }
