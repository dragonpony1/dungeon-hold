...
function staveCurve(L,recurve){ const k=L/1.5; if(!recurve) return new THREE.QuadraticBezierCurve3(new THREE.Vector3(0,0,0),new THREE.Vector3(0,L/2,.34*k*2),new THREE.Vector3(0,L,0));   // control point at 2× the bulge: the curve peaks at .34
  const pts=[[0,.08],[.1,-.02],[.3,.22],[.5,.34],[.7,.22],[.9,-.02],[1,.08]].map(([u,z])=>new THREE.Vector3(0,u*L,z*k)); return new THREE.CatmullRomCurve3(pts,false,'catmullrom',.5); }
// the bow is two limbs pivoting at the grip (so a draw can bend them back toward the archer), the grip and its fittings between
function makeBow(kind){ const K=BOW_KINDS[kind]||BOW_KINDS.ash; const g=new THREE.Group(); g.name='bow-'+kind; const wood=mat(K.wood), dark=mat(K.dark), band=mat(K.band), bright=h=>basic(h); const L=1.5*(K.len||1); const curve=staveCurve(L,K.recurve); const P=u=>curve.getPoint(u); const gp=P(.5);
  const limbU=new THREE.Group(), limbL=new THREE.Group(); limbU.name='limbU'; limbL.name='limbL'; limbU.position.set(0,gp.y,gp.z); limbL.position.copy(limbU.position); g.add(limbU,limbL);
  { const pts=curve.getPoints(28); for(const [limb,sub] of [[limbL,pts.slice(0,15)],[limbU,pts.slice(14)]]){ const geo=new THREE.TubeGeometry(new THREE.CatmullRomCurve3(sub,false,'catmullrom',.5),16,.026,6,false); geo.translate(-gp.x,-gp.y,-gp.z); limb.add(new THREE.Mesh(geo,wood)); } }   // the stave, split at the grip
  const at=(mesh,u,dz)=>{ const p=P(u); mesh.position.set(0,p.y-gp.y,p.z-gp.z+(dz||0)); (u>.5?limbU:limbL).add(mesh); return mesh; };
  g.add(M(G.cyl(.04,.04,.24,8),dark,0,gp.y,gp.z)); for(const dy of [-.06,0,.06]) g.add(M(G.cyl(.043,.043,.014,8),band,0,gp.y+dy,gp.z));                // the leather grip and its three rings
  for(const u of [.08,.92]) at(M(G.cyl(.03,.03,.03,8),band),u);                                                                            // bands near the tips
  if(K.wraps) for(const u of [.24,.76]) at(M(G.cyl(.032,.032,.1,8),dark),u);                                                                // leather wraps down the limbs
  if(K.plates) for(const u of [.15,.22,.29,.71,.78,.85]) at(M(G.cyl(.031,.031,.035,8),band),u);                                             // horn plates
  for(const u of [0,1]){ let t; if(K.tips==='gold'){ t=M(G.cone(.03,.09,6),band); t.rotation.x=u?0:PI; } else if(K.tips==='crystal'){ t=new THREE.Mesh(new THREE.OctahedronGeometry(.035,0),bright(K.gem)); t.scale.set(1,1.8,1); t.userData.noOL=true; } else t=M(G.sph(.03,6,5),band); at(t,u,0); }   // the nocks: horn knobs, gold caps or void crystals
  for(const i of [0,1]){ const str=M(G.cyl(K.litString?.007:.005,K.litString?.007:.005,1,4),K.litString?bright(K.glow):dark); str.name='string'+i; if(K.litString) str.userData.noOL=true; g.add(str); }   // the string in two halves, tip to nock (a magic bow's burns)
  ...
  if(K.runes) for(let i=0;i<3;i++){ const r=at(M(G.box(.012,.06,.008),bright(K.glow)),.28+i*.06,.028); r.userData.noOL=true; r.name='rune'+i; }   // runes down the lower limb
  if(K.gem){ const gem=new THREE.Mesh(new THREE.OctahedronGeometry(.045,0),bright(K.gem)); gem.name='gem'; gem.scale.set(1,1.6,1); gem.position.set(0,gp.y,gp.z+.075); gem.userData.noOL=true; g.add(gem); }   // a crystal set in the belly
...
const _Y=new THREE.Vector3(0,1,0), _nock=new THREE.Vector3(), _tip=new THREE.Vector3(), _dir=new THREE.Vector3(); const FLEX=.22;
function setDraw(root,pull){ const L=root.userData.L||1.5, zs=root.userData.zs||0, gz=root.userData.gz||0, gy=L/2; const flex=pull*FLEX; const U=root.getObjectByName('limbU'), D=root.getObjectByName('limbL'); if(U) U.rotation.x=-flex; if(D) D.rotation.x=flex;
  _nock.set(0,gy,zs-pull*.42); for(const i of [0,1]){ const st=root.getObjectByName('string'+i); if(!st) continue; const y=(i?1:-1)*L/2, z=zs-gz, r=i?-flex:flex; _tip.set(0,gy+y*Math.cos(r)-z*Math.sin(r),gz+y*Math.sin(r)+z*Math.cos(r)); _dir.copy(_nock).sub(_tip); const len=_dir.length(); st.position.copy(_tip).lerp(_nock,.5); st.scale.set(1,len,1); st.quaternion.setFromUnitVectors(_Y,_dir.normalize()); }   // a tip after its limb's bend (a turn about the grip's x axis)
  const n=root.getObjectByName('nocked'); if(n){ n.visible=pull>.05; n.position.set(0,gy,_nock.z+ARROW_L/2); } }
