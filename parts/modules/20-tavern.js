...
.tv-sub .tv-n{color:#c9b8a0;letter-spacing:0;font-weight:normal;font-size:12px;min-width:0;overflow:hidden;text-overflow:ellipsis} .tv-sub .sp{flex:1}.tv-sub{flex-wrap:wrap;row-gap:6px}.tv-grp{grid-column:1/-1;font-size:12px;letter-spacing:2px;color:var(--gold);font-weight:bold;margin:6px 0 -4px}.tv-btn.sm{min-height:34px;padding:4px 10px;font-size:11px;letter-spacing:.5px}.tv-lock{position:absolute;top:4px;right:6px;font-size:12px;pointer-events:none}
...
function tvCard(it,from,extra){ const sel=TV.sel&&TV.sel.id===it.id; return '<div class="tv-card'+(sel?' sel':'')+'" data-act="sel" data-id="'+it.id+'" data-from="'+from+'"><span class="ic">'+SICON[it.slot]+'</span>'+(it.locked?'<span class="tv-lock" title="Locked: never scrapped at the portal or sold as junk">🔒</span>':'')+'<div class="nm" style="color:'+RCSS[it.rarity]+'">'+tvTier(it)+tvEsc(it.name)+'</div><div class="st">'+tvEsc(statStr(it))+'</div>'+(extra||'')+'</div>'; }
...
function tvRenderDetail(){ const el=$('tv-detail'), body=$('tv-body'); const s=TV.sel; if(!s){ el.classList.add('hide'); body.classList.remove('det'); return; }
  ...
  else if(s.from==='bag') b='<button class="tv-btn hot" data-act="equip" data-id="'+it.id+'">Equip</button><button class="tv-btn" data-act="lock" data-id="'+it.id+'" title="A locked piece is never scrapped at the portal or sold as junk">'+(it.locked?'🔓 Unlock':'🔒 Lock')+'</button><button class="tv-btn" data-act="sell" data-id="'+it.id+'"'+(it.locked?' disabled title="Unlock it to sell"':'')+'>Sell ('+tvG(it.value)+' gold)</button>';
...
function tvClick(e){ const t=e.target.closest('[data-act]'); if(!t||t.disabled) return; const a=t.dataset.act, D=t.dataset;
  ...
  else if(a==='lock'){ const on=Meta.toggleLock(D.id); if(on!==null) tvSay(on?'Locked — it stays out of the portal\'s scrap and the junk sale':'Unlocked'); tvRenderTab(true); }
