...
const maint=await page.evaluate(()=>{ const el=document.getElementById('maintline'); const vis=e=>!!e&&(e.checkVisibility?e.checkVisibility():e.offsetParent!==null); const btns=[...(el?el.querySelectorAll('button'):[])].map(b=>b.textContent); const kids=[...document.getElementById('start').children]; return {vis:vis(el),btns,aboveBuild:kids.indexOf(el)===kids.indexOf(document.getElementById('buildline'))-1,inTest:!!document.querySelector('#testline [data-t=wipe]')}; });
check("the update (fresh reload) and wipe-saves buttons stay in plain sight on their own line just above the build line, not in the fold",maint.vis&&maint.btns.some(t=>/update|fresh reload/.test(t))&&maint.btns.some(t=>/wipe saves/.test(t))&&maint.aboveBuild&&!maint.inTest,JSON.stringify(maint));
