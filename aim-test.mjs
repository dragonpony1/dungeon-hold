...
check("a tap looses weak: about 60% damage",r4.last<.4&&r4.lost>0&&r4.lost<=r4.dmg*.9,JSON.stringify(r4));   // the real mousedown/mouseup round trip carries a little charge even on a tap — more of a shorter fullT() now that a full charge comes round faster
...
await page.evaluate(()=>{ const d=window.__dd; for(const e of d.enemies) d.kill(e); d.setHero(0,10,0); d.hero.y=0; d.setCam(0,.42,6); d.step(1/60,5); const a=d.spawn("goblin","N"); a.x=0; a.z=17; a.spd=0; a.hp=a.max=500; const b=d.spawn("goblin","N"); b.x=1.3; b.z=17.4; b.spd=0; b.hp=b.max=500; window.__pair=[a,b]; d.step(1/60,2); });
await down();
