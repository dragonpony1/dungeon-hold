...
// dist/hideout/vendor/GLTFLoader.js gets the same texture fix the game's own loader carries (parts/head.html): an
// embedded texture is decoded straight from its bytes with createImageBitmap instead of being turned into a blob: URL
// and fetched, because a strict Content-Security-Policy in an embedding viewer (the artifact host: connect-src 'self')
// refuses that fetch, every model then fails to parse, and the room never builds. The source copy stays stock; both
// rewrites are anchored and fail the build if the vendor file changes shape.
function patchHideoutLoader(js){
  const a=/isObjectURL = true;\s*const blob = new Blob\( \[ bufferView \], \{\s*type: source\.mimeType\s*\} \);\s*sourceURI = URL\.createObjectURL\( blob \);\s*return sourceURI;/g;
  if((js.match(a)||[]).length!==1) throw new Error("hideout GLTFLoader: the embedded-image blob site not found exactly once -- vendor file changed?");
  js=js.replace(a,"const blob = new Blob( [ bufferView ], { type: source.mimeType } );\n\t\t\t\t\t/* patched by assemble.mjs: decode from the bytes, no object URL and no fetch, so a strict CSP cannot block the texture */\n\t\t\t\t\tif ( typeof createImageBitmap === 'function' ) { try { return createImageBitmap( blob, { premultiplyAlpha: 'none', colorSpaceConversion: 'none' } ).then( function ( bmp ) { return { __bitmap: bmp }; }, function () { isObjectURL = true; sourceURI = URL.createObjectURL( blob ); return sourceURI; } ); } catch ( e ) {} }\n\t\t\t\t\tisObjectURL = true;\n\t\t\t\t\tsourceURI = URL.createObjectURL( blob );\n\t\t\t\t\treturn sourceURI;");
  const b=/return Promise\.resolve\( sourceURI \)\.then\( function \( sourceURI \) \{\s*return new Promise\( function \( resolve, reject \) \{/g;
  if((js.match(b)||[]).length!==1) throw new Error("hideout GLTFLoader: the texture promise site not found exactly once -- vendor file changed?");
  js=js.replace(b,"return Promise.resolve( sourceURI ).then( function ( sourceURI ) {\n\t\t\t\tif ( sourceURI && sourceURI.__bitmap ) return new THREE.CanvasTexture( sourceURI.__bitmap ); /* patched by assemble.mjs */\n\t\t\t\treturn new Promise( function ( resolve, reject ) {");
  return js; }
...
if(process.env.DIST){ const D=process.env.DIST; fs.mkdirSync(D+"/assets",{recursive:true}); const crypto=await import("crypto"); for(const f of fs.readdirSync(P+"/assets")){ if(/\.glb$/.test(f)){ const raw=fs.readFileSync(P+"/assets/"+f), b64=raw.toString("base64"), st=crypto.createHash("sha1").update(raw).digest("hex").slice(0,8); fs.writeFileSync(D+"/assets/"+f+".txt",b64); fs.writeFileSync(D+"/assets/"+f.replace(/\.glb$/,"")+"."+st+".glb.txt",b64); } else fs.copyFileSync(P+"/assets/"+f,D+"/assets/"+f); } outPath=D+"/index.html"; if(fs.existsSync(P+"/hideout")){ fs.cpSync(P+"/hideout",D+"/hideout",{recursive:true}); glbToTxt(D+"/hideout/assets"); fs.writeFileSync(D+"/hideout/vendor/GLTFLoader.js",patchHideoutLoader(fs.readFileSync(D+"/hideout/vendor/GLTFLoader.js","utf8"))); const hh=fs.readFileSync(P+"/hideout/index.html","utf8"); fs.writeFileSync(D+"/hideout/index.html",embedHideout(hh)); const hb=(hh.match(/<meta name="hideout-build" content="(\d+)"/)||[])[1]; if(!hb) throw new Error("hideout: no <meta name=hideout-build> in the upstream page -- it carries one since hideout build 9"); page=page.replace("const HIDEOUT_BUILD=/*HIDEOUT*/0;","const HIDEOUT_BUILD=/*HIDEOUT*/"+hb+";"); } }   // the hideout (parts/hideout/: its own page, vendor/ and models) rides along as dist/hideout/, opened by the game in an overlay (59-hideout.js)   // models are written twice: plain (the fallback) and with their content stamp in the name (what the page asks for)
