...
{ const frame=await frameOf(page,'hideout/index.html');
  ...
  const shown=await frame.evaluate(id=>{ buildLockerGrids(); /* hideout build 10: carried trophies live in the Arcane Wardrobe's 'Locked & carried in' grid, not YOUR GEAR */ const cards=[...document.querySelectorAll('#lockerCarried .fgear')].map(el=>({name:(el.querySelector('.fname')||{}).textContent,sub:(el.querySelector('.fsub')||{}).textContent})); const piece=allPieces().find(p=>p.kind==='carried'&&p.id===id); return {cards,piece:piece&&{kind:piece.kind,type:piece.type,tier:piece.tier,label:piece.label}}; },ids.mythic);
  check("the hideout's wall locker (hideout build 10, the home of carried trophies) lists the carried mythic by name, as a legendary weapon",!!shown.piece&&shown.piece.kind==='carried'&&shown.piece.type==='weapon'&&shown.piece.tier==='legendary'&&shown.cards.some(c=>c.name==='Emberfang, the Cool One'&&/legendary weapon/i.test(c.sub||'')),JSON.stringify(shown));
