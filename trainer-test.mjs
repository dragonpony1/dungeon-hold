...
check("the ballista on the lane ticks step 3; step 4 is the horn",v3.step==='horn'&&/horn/i.test(v3.text)&&v3.n==='4/9',JSON.stringify(v3));
...
check("equipping ticks step 7; the locker step ticks itself off (nothing waits there); step 9 asks for a second defense or an upgrade",v7.step==='more'&&v7.counts.equips>0&&/second defense|Mark II/.test(v7.text),JSON.stringify(v7));
...
check("after a reload the guide resumes at 9/9 (progress kept in localStorage dd_trainer)",v8.on&&v8.step==='more'&&v8.n==='9/9',JSON.stringify(v8));
