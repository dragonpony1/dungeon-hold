...
const extra=await page.evaluate(async()=>{ if(!window.__mus||!window.__defglb) return null; window.__defglb.ensure('harpoon',2); /* the marks past I are lazy since build 123: ask for Mark II so a lazily fetched model is proven to load under CSP too */ for(let i=0;i<100&&!(window.__mus.state().fetched.length>=2&&window.__defglb.list().harpoon&&window.__defglb.list().harpoon[1]);i++) await new Promise(r=>setTimeout(r,200)); return {music:window.__mus.state().fetched,ballista:(window.__defglb.list().harpoon||[]).length}; });
