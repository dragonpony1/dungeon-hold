// node merge.mjs [query]   — merges rig.glb + clips into witch.glb with a staff mount (see merge.html for the parameters)
...
await page.goto("http://127.0.0.1:8791/merge.html?"+(process.argv[2]||"")); await page.waitForFunction(()=>window.build);
const out=await page.evaluate(()=>window.build().catch(e=>({error:String(e&&e.stack||e)})));
if(out.b64){ fs.writeFileSync("witch.glb",Buffer.from(out.b64,"base64")); delete out.b64; }
