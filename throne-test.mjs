...
const ready=()=>page.waitForFunction(()=>window.__dd&&window.__dd.map&&window.__campaign&&window.__dd.heroModel()&&window.__dd.mobModel("goblin"),null,{timeout:120000});
...
const r=await page.evaluate(()=>{ const d=window.__dd; window.__meta.reset(); d.resetGear(); d.start(); d.step(1/60,5); const L=d.lanes(); const m=d.map();
  ...
  return {gw:m.gw,gh:m.gh,wallH:m.wallH,windows:m.windows,rails:window.__dd.rails?window.__dd.rails():null,lanes:Object.keys(L),from:Object.fromEntries(Object.keys(L).map(k=>[k,L[k].from])),paths,flyPaths,reach,gates,H,heroY,hasRoom:!!window.__room,line:window.__campaign.line(),wbase:m.wbase}; });
...
check("hero spawns on the throne platform eight up; windows and rails built; no physical tavern room on this map; wbase 7",r.heroY>=7.9&&r.windows>=6&&!r.hasRoom&&r.wbase===7&&/MAP 2 OF 5/.test(r.line),JSON.stringify({heroY:r.heroY,windows:r.windows,hasRoom:r.hasRoom}));
...
check("defenses stand on the floor, all three landings and the top, and on a flight they stand on that cell's upper step (first flight 1.5, middle flight 3.5)",r3.floor===0&&r3.landingA===2&&r3.landingB===4&&r3.landingC===6&&r3.top===8&&r3.flight===1.5&&r3.flight2===3.5,JSON.stringify(r3));
// screenshots
