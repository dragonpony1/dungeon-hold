...
const twin=await page.evaluate(()=>{ const d=window.__dd, F=window.__forest, M=window.__meta; for(const e of d.enemies) e.dead=true; d.setHero(0,10,0); d.step(1/60,30); const boon=F.boon(); const pet=window.__familiar.state();
  ...
  const e=d.spawn('goblin','N'); e.hp=1e9; e.spd=0; e.x=pet.x+1; e.z=pet.z+5; const b0=window.__familiar.bolts(); let maxBolts=0; for(let i=0;i<40;i++){ d.step(1/60,1); e.x=pet.x+1; e.z=pet.z+5; maxBolts=Math.max(maxBolts,window.__familiar.bolts()); }
  return {boon,pet:!!pet,kind:pet&&pet.kind,base:+base.toFixed(3),rate:rate===null?null:+rate.toFixed(3),targetsFar,maxBolts,b0}; });
check("the boon is on the pack (twin, rate 1.5, range 1.5) and a familiar is out",!!twin.boon&&twin.boon.twin&&twin.boon.rate===1.5&&twin.boon.range===1.5&&twin.pet,JSON.stringify({boon:twin.boon,kind:twin.kind}));
check("the familiar targets a goblin 12 units out (its reach is 9 without the set) and a shot puts two bolts in the air",twin.targetsFar&&twin.maxBolts>=2,JSON.stringify({far:twin.targetsFar,maxBolts:twin.maxBolts,kind:twin.kind}));
...
const guar=await page.evaluate(()=>{ const d=window.__dd, F=window.__forest, M=window.__meta; M.reset(); d.resetGear(); for(const l of d.loot) l.mesh.parent&&l.mesh.parent.remove(l.mesh); d.loot.length=0; d.setHero(30,30,0);
  ...
  const n3=F.guarantee(3); const a3={own:own(),floor:floor(),n:n3,slots:Object.keys(F.owned()).sort()}; for(const l of [...d.loot]) M.onPickup(l.it); d.loot.length=0; const n4=F.guarantee(4);
  return {a1,a1b,a2,a3,n4,byWave:F.BY_WAVE}; });
check("wave 1 held: two Forest pieces drop by the crystal (a second call adds none while they lie there); wave 2: two more; wave 3: the last -- all five slots owned; wave 4: nothing more",guar.a1.n===2&&guar.a1.floor===2&&guar.a1b.n===0&&guar.a2.n===2&&guar.a2.own===4&&guar.a3.n===1&&guar.a3.own===5&&guar.a3.slots.join()==='amulet,armor,charm,familiar,weapon'&&guar.n4===0,JSON.stringify(guar));
