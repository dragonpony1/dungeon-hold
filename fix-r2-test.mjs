...
const leak=await page.evaluate(async()=>{ const d=window.__dd, M=window.__meta; M.reset(); d.resetGear(); d.start(); d.step(1/60,2); for(let i=0;i<200&&!(window.__weapons&&window.__weapons.state().mounted&&/Meshy/.test(d.heroModel().label)&&d.mobModel('ogre'));i++){ d.step(1/60,1); await new Promise(r=>setTimeout(r,50)); } d.step(1/60,5); d.shot(64,64); const g0=d.r.info.memory.geometries; /* lazily fetched hero, sword and mob models have landed */ const kinds=['Wisp','Bat','Sprite','Fire Imp','Crystal Owl','Storm Drake']; for(let i=0;i<12;i++){ const it=d.rollItem(2,'familiar',3); it.name='Fine '+kinds[i%6]; M.giveItem(it); M.equip(it.id); d.step(1/60,2); d.shot(64,64); } const g1=d.r.info.memory.geometries; M.unequip('familiar'); d.step(1/60,2); d.shot(64,64); return {g0,g1,g2:d.r.info.memory.geometries}; });
