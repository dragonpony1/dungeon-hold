...
const r1=await page.evaluate(()=>{ const d=window.__dd; window.__meta.reset(); d.resetGear(); d.start(); d.addMana(9000); d.setHero(8,10,0); d.step(1/60,3);
  ...
  const yoke=t.mdl.userData.yoke; return {drakeY:+dr.y.toFixed(2),pitch:+pitchMax.toFixed(2),yokeX:+yoke.rotation.x.toFixed(2),bolt,hitAt,hp:[hpBefore,dr.hp]}; });
check("the ballista pitches up at a drake (yoke tilts, the bolt climbs) and the bolt hits it",r1.pitch>.12&&r1.yokeX<-.1&&r1.bolt&&r1.bolt.vy>2&&r1.hitAt>=0&&r1.hp[1]<r1.hp[0],JSON.stringify(r1));
